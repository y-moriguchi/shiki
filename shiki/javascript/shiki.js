	...
	function shiki(input, option) {
		...
		function quadro() {
			...
			me.markRootSubPowProcessedLeft = function() {
				var i,
					cell;
				if(!me.get().markRootWall) {
					return;
				}
				me.get().markRootProcessed = true;
				for(i = 1;; i++) {
					cell = getPosRel(-i, 0);
			...
			me.markRootIgnoreUpperWall = function() {
				var wall,
					i,
					cell,
					ch;
			...
			me.transit = function(state, quadro) {
				...
				case st.FPRINTABLE_V_COUNT:
					if(quadro.isAboveSlashRootWall()) {
						if(quadro.isRootNotation()) {
							quadro.markRootIgnoreUpperWall();
				...
				case st.ROOT_SCAN:
					if(quadro.isMatrixByScanningDown() || quadro.isSumInt(1)) {
						...
						quadro.get().markNumberOfRootV.push(quadro.countRootWalls);
				...
				case st.FRET:
					...
					case 'ROOTBASE':
						v2 = quadro.popModel();
						v1 = quadro.popModel();
						quadro.addModel(new Root(v1, v2, cell.markNumberOfRootV.pop()));
						quadro.clearStringBuilder();
